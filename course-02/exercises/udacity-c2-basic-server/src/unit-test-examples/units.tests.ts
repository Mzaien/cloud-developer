...
describe("Concat two strings into one",()=>{

it('Add Apple and gold',()=>{
  const result=concat('Apple','gold');
  expect(result).to.equal('Applegold')
});

it('Add Apple and nothing which throw and error',()=>{
  
  expect(()=>{concat('Apple')}).to.throw('Please add two words')
});
});